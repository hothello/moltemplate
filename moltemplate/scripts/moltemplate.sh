...
if [ -s "${postprocess}.template" ]; then
    if ! $PYTHON_COMMAND "${PY_SCR_DIR}/alchemical.py" \
                                    "${postprocess}.template" "${data_atoms}.template" \
                                    "${data_ellipsoids}.template" "$TTREE_ARGS"; then
        ERR_INTERNAL
    fi
    # Override the files generated by LTTREE_COMMAND
    if [ -s "${data_ellipsoids}" ]; then
        files=("${data_atoms}" "${data_ellipsoids}")
    else
        files=("${data_atoms}")
    fi
    for f in "${files[@]}"; do
